/*
	...
	This is calculated by looking for border items, or in the case of clicking diagonally from yourself,
		dense items.
...
/turf/Adjacent(var/atom/neighbor, var/atom/target = null)
	...
	if(get_dist(src, T0) > 1 || (T0.z != src.z))
		return FALSE
	...
	var/d1 = in_dir&(EAST|WEST)		//get east-west component
	var/d2 = in_dir&(NORTH|SOUTH)	//get south-north component
...
*/
/turf/proc/ClickCross(var/target_dir, var/border_only, var/target_atom)
	for(var/obj/O in src)
		if(!O.density || O == target_atom)
			continue
